...
internal class TransformatingMazeTests {
    ...
    fun rotateCell(cellValue: Int, rotations: Int): Int {
        var _cellValue = cellValue
        if (_cellValue <= 0)
            return _cellValue
        for (i in 1..rotations)
            _cellValue = (_cellValue shl 1) or (_cellValue shr 3) and 0xf
        return _cellValue
    }
    ...
    fun findPath(maze: Array<IntArray>, x:Int, y:Int, rotations:Int, results:MutableList<List<String>>, directions:List<String> = listOf(""), start: Long = System.currentTimeMillis()) {
...
//            return
        ...
        Direction.values().forEach { direction ->
            makeStep(maze, x, y, direction)?.also {
                ...
                _maze[x][y] = -1
                findPath(_maze, it.first, it.second, 0, results, _directions, start)
            }
        }


        if (rotations < 3)
            findPath(rotateMaze(maze), x, y, rotations + 1, results,directions + "",  start)
    }
    ...
    fun isSolvable(maze: Array<IntArray>, startPoint: Pair<Int, Int>): Boolean {
        ...
        findPath(_maze, startPoint.first, startPoint.second, 0, results)
...
//        println(maze.map { it.joinToString(",") }.joinToString("\n"))
        ...
        var results = mutableListOf<List<String>>()
        findPath(maze, startPoint.first, startPoint.second, 0, results)
    ...
    @Test
    fun NoSolutionPuzzles() {
        val testCases = arrayOf(
            Pair(
                arrayOf(
                    intArrayOf(9, 2, 2, 2),
                    intArrayOf(15, 10, 8, 6),
                    intArrayOf(13, 7, 5, 0),
                    intArrayOf(-1, 5, 3, -2),
                ), null
            ),
...
//            ),
        ...
        testCases.forEach { (maze, sol) ->
            ...
            assertTrue(
                result.contentDeepEquals(sol),
                "Result:  ${result?.joinToString(",") ?: "null"} \nSolution: ${/*sol?.joinToString(",") ?:*/ "null"}"
            )
        }
    }
